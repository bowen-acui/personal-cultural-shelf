...
function parseItem(value) {
  ...
  if (value.aspectRatio !== undefined && (typeof value.aspectRatio !== "number" || !Number.isFinite(value.aspectRatio) || value.aspectRatio <= 0)) return null;
...
export function validateMediaData(value) {
  ...
  const records = [];
  const invalid = [];
  // 单条记录格式坏不该让整站白屏：跳过并报出 id，其余照常展示。
  value.forEach((item, index) => {
    const parsed = parseItem(item);
    if (parsed) records.push(parsed);
    else invalid.push(item?.id ?? `#${index}`);
  });
  if (invalid.length) console.warn(`媒体数据包含 ${invalid.length} 条无效记录，已跳过：${invalid.join(", ")}`);
  // 重复 ID 是构建 bug，不是脏数据，该炸就炸。
