...
function itemRatio(item, type, viewportWidth = 1024) {
  const ratio = Number(item?.aspectRatio);
  return Number.isFinite(ratio) && ratio > 0 ? ratio : dimensions(viewportWidth, type).ratio;
}
...
export function createScatterLayout(items, viewport, seed = 0, type = "book") {
  ...
  return items.map((item, index) => {
    ...
    return {
      ...
      scale: 1,
      ratio: itemRatio(item, type, viewport.width),
    };
  });
}
