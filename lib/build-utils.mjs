...
export function hasCompletedBook(metadata) {
  return Boolean(metadata.完读日期)
    || /^(已读|读完|完成|读完待整理)$/.test(metadata.状态?.trim() ?? "");
}

export function publicationDecision(type, metadata) {
  const title = String(metadata.名称 || metadata.书名 || "").trim();
  const cover = String(metadata.封面 || "").trim();
  if (type === "book" && !hasCompletedBook(metadata)) return { status: "skip", reason: "not-completed" };
  if (!title && type !== "book") return { status: "error", reason: "missing-title" };
  if (!cover) return { status: "error", reason: "missing-cover" };
  return { status: "include", reason: "ready" };
}

export function isDisplayableBook(metadata) {
  return publicationDecision("book", metadata).status === "include";
}
...
export function isDisplayableMedia(metadata) {
  return publicationDecision("media", metadata).status === "include";
}
...
const MUSIC_ARTIST_ALIASES = new Map([
  ["eason chan", "陈奕迅"],
  ["陳奕迅", "陈奕迅"],
  ["陳嫺靜", "陈娴静"],
  ["陈嫺静", "陈娴静"],
...
export function musicArtistKey(value) {
  const normalized = String(value ?? "").trim().normalize("NFKC").toLocaleLowerCase("zh-CN");
  return MUSIC_ARTIST_ALIASES.get(normalized) ?? normalized;
}

export function artistRank(metadata) {
  const raw = String(metadata.艺人顺序 ?? "").trim();
  const rank = Number(raw);
  return raw && Number.isFinite(rank) ? rank : 99;
}

export function sortMediaRecords(records) {
  const musicGroupRanks = new Map();
  records.filter((record) => record.type === "music").forEach((record) => {
    const key = musicArtistKey(record.creator);
    const current = musicGroupRanks.get(key) ?? 99;
    musicGroupRanks.set(key, Math.min(current, record.artistRank ?? 99));
  });
  return records.sort((left, right) => {
    ...
    if (left.type === "music") {
      const creatorOrder = musicArtistKey(left.creator).localeCompare(musicArtistKey(right.creator), "zh-CN");
      const groupOrder = (musicGroupRanks.get(musicArtistKey(left.creator)) ?? 99)
        - (musicGroupRanks.get(musicArtistKey(right.creator)) ?? 99);
      if (groupOrder) return groupOrder;
      if (creatorOrder) return creatorOrder;
      const priorityOrder = (left.pinned ?? 99) - (right.pinned ?? 99);
      if (priorityOrder) return priorityOrder;
    }
...
export function toPublicMedia(filename, metadata, publicCover, type, publicCoverLarge = "", aspectRatio = 0) {
