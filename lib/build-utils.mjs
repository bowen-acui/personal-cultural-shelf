...
function musicArtistKey(value) {
  const artist = String(value ?? "").trim();
  return /(?:Eason Chan|陈奕迅|陳奕迅)/i.test(artist) ? "eason chan" : artist.toLocaleLowerCase("zh-CN");
}
...
export function sortMediaRecords(records) {
  return records.sort((left, right) => {
    ...
    if (left.type === "music") {
      const leftEason = /(?:Eason Chan|陈奕迅|陳奕迅)/i.test(left.creator ?? "") ? 0 : 1;
      const rightEason = /(?:Eason Chan|陈奕迅|陳奕迅)/i.test(right.creator ?? "") ? 0 : 1;
      const artistOrder = leftEason - rightEason;
